...
class AgenticSearch {
    ...
    handleWindowResize() {
        ...
        // Optional: Close sidebar if window becomes too small
        if (windowWidth < 600 && this.sidebar.classList.contains('show')) {
            this.closeMobileSidebar();
        }
    }
    ...
    async sendSearch() {
        ...
        try {
            ...
            // Add followup button after search completes successfully
            if (this.currentSessionId && this.currentQueryContainer) {
                setTimeout(() => this.addFollowupButton(), 500); // Small delay to ensure response is fully rendered
            }
        }
    }
    ...
    addFollowupButton() {
        // Only add followup button if we have a current session and the query container exists
        if (!this.currentSessionId || !this.currentQueryContainer) return;

        // Check if followup button already exists in this container
        if (this.currentQueryContainer.querySelector('.followup-button')) return;

        const followupContainer = document.createElement('div');
        followupContainer.classList.add('followup-container');

        const followupButton = document.createElement('button');
        followupButton.classList.add('followup-button');
        followupButton.innerHTML = '💬 Ask Followup Question';
        followupButton.addEventListener('click', () => this.startFollowupQuery());

        followupContainer.appendChild(followupButton);
        this.currentQueryContainer.appendChild(followupContainer);
        this.scrollToBottom();
    }

    startFollowupQuery() {
        // Focus on the message input for followup query
        this.messageInput.focus();
        this.messageInput.placeholder = "Ask a followup question...";
    }
