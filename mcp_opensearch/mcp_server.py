#!/usr/bin/env python3
"""
MCP OpenSearch Server - Fully Compliant with 2025-06-18 Specification
Implements Streamable HTTP transport for OpenSearch queries via HTTP
"""
import asyncio
import json
import uuid
...
class MCPServer:
    ...
    def __init__(self, opensearch_url: str = "http://localhost:9200"):
        self.tools = MCPTools(opensearch_url=opensearch_url)
...
app = FastAPI(
    ...
    version="1.0.0",
    description="MCP server for OpenSearch with HTTP-based queries (2025-06-18 compliant)"
)

# Initialize MCP server instance
mcp_server = MCPServer(opensearch_url=OPENSEARCH_URL)
...
async def root():
    ...
    return {
        ...
        "description": "MCP server for OpenSearch with HTTP-based queries",
        "opensearch_url": OPENSEARCH_URL,
        "endpoints": {
...
async def mcp_get_endpoint(
    ...
    try:
        ...
        async def server_initiated_stream():
            """Generate SSE events for server-to-client communication."""
            try:
