#!/usr/bin/env python3
"""
MCP OpenSearch Server Implementation - Fully Compliant with 2025-06-18 Specification
Implements Streamable HTTP transport with OpenSearch integration capabilities
"""
import asyncio
import json
import uuid
...
# Import our OpenSearch tools system
from tools import MCPOpenSearchTools
...
class MCPOpenSearchServer:
    """
    MCP OpenSearch Server implementation fully compliant with 2025-06-18 Streamable HTTP transport.
    Implements all required security features and protocol requirements with OpenSearch integration.
    """

    def __init__(self):
        # Initialize OpenSearch tools with default localhost connection
        opensearch_hosts = [os.getenv("OPENSEARCH_HOST", "localhost:9200")]
        self.tools = MCPOpenSearchTools(opensearch_hosts=opensearch_hosts)
        ...
        logger.info(f"MCP OpenSearch Server initialized with protocol version {PROTOCOL_VERSION}")
        logger.info("OpenSearch integration and compliance components initialized")
    ...
    async def handle_initialize(
        ...
        logger.info(f"MCP OpenSearch Server initialized by {client_name} with session {session_id}")
    ...
    async def handle_tools_call(self, params: Dict[str, Any], request_id: str, session_id: Optional[str] = None) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            logger.error(f"Error during OpenSearch tool execution '{tool_name}': {e}", exc_info=True)
            error_event_id = str(uuid.uuid4())
            yield f"id: {error_event_id}\n"
...
app = FastAPI(
    title="MCP OpenSearch Server",
    version="1.0.0",
    description="Fully compliant with MCP 2025-06-18 specification with OpenSearch integration"
)

# Initialize MCP OpenSearch server instance
mcp_server = MCPOpenSearchServer()
...
async def root():
    ...
    return {
        ...
        "description": "OpenSearch integration for MCP",
        "endpoints": {
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting MCP OpenSearch Server (2025-06-18 compliant) on {host}:{port}...")
    logger.info(f"Protocol Version: {PROTOCOL_VERSION}")
    logger.info(f"Server Info: {SERVER_INFO}")
    logger.info(f"OpenSearch connection target: {os.getenv('OPENSEARCH_HOST', 'localhost:9200')}")
